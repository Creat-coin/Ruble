...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Ruble " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"rubletools.com", "188.165.235.208"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Ruble is probably already running."), addrBind.ToString().c_str());
        else
