...
#if defined(NDEBUG)
# error "Ruble cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x45e35546358dd596e19d40f769133519cc7eb4cb7442c4042ff06c9c6654a591");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ruble: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ruble Signed Message:\n";
...
{
    // Ruble: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Ruble: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Ruble: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ruble: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ruble: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x45e35546358dd596e19d40f769133519cc7eb4cb7442c4042ff06c9c6654a591");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "10/JUNE/2017 Attentat de Londres: la une de Charlie Hebdo choque les Britanniques";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("483494318fa689ad5023690c80f3a49c8f13f8d75b8c654fbcbc8bc5a8e4d3eb4b10f4d4604fa08dce601aaf0f475496fe1b37550b4acf21b179c45070ac7b03a7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1497072230;
        ...
        assert(block.hashMerkleRoot == uint256("0x6e6feb1e1c5ca6c160a9d5d1405e0117a2bd05c698647c7d3060ee240d0f0e4a"));
...
unsigned char pchMessageStart[4] = { 0xc3, 0xd2, 0xd1, 0xbd }; // Ruble: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RubleMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RubleMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RubleMiner(CWallet *pwallet)
{
    printf("RubleMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ruble-miner");
    ...
    try { loop {
        ...
        printf("Running RubleMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RubleMiner, pwallet));
}
